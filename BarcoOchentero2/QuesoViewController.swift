...
class QuesoViewController: UIViewController {
    ...
    @IBAction func paso4Action(_ sender: Any) {
        viewCheck()
        if(estado == true){
            
            sendInformation()
            self.performSegue(withIdentifier: "paso4Push", sender: QuesoViewController.self)
        }else{
        ...
        func sendInformation(){
            if(check1.isOn){
                ref.child("configuracion").child("queso").child("precio").setValue(   precioLabel1.text  )
                ref.child("configuracion").child("queso").child("seleccion").setValue(nombreLabel1.text )
            }
            if(check2.isOn){
                ref.child("configuracion").child("queso").child("precio").setValue(   precioLabel2.text  )
                ref.child("configuracion").child("queso").child("seleccion").setValue(nombreLabel2.text )
            }
            if(check3.isOn){
               ref.child("configuracion").child("queso").child("precio").setValue(   precioLabel3.text  )
                ref.child("configuracion").child("queso").child("seleccion").setValue(nombreLabel3.text )
            }
            if(check4.isOn){
               ref.child("configuracion").child("queso").child("precio").setValue(   precioLabel4.text  )
                ref.child("configuracion").child("queso").child("seleccion").setValue(nombreLabel4.text )
            }
